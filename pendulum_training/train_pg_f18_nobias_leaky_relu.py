...
#========================================================================================#  
def my_leaky_relu(x):
    return tf.nn.leaky_relu(x, alpha=0.6)
...
class Agent(object):
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_logprob_n = fancy_slice_2d(sy_logp_na, tf.range(tf.shape(sy_ac_na)[0]), sy_ac_na)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            EPSILON=1e-8
            dist = tf.distributions.Normal(loc=sy_mean,scale=tf.exp(sy_logstd+EPSILON),validate_args=True)
            sy_logprob_n = tf.reduce_sum(dist.log_prob(sy_ac_na),axis=1) # log_prob is actually log(pdf)
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            # raise NotImplementedError
            self.baseline_prediction = tf.squeeze(build_mlp_baseline(
                                    self.sy_ob_no, 
                                    1, 
                                    "nn_baseline",
                                    n_layers=self.n_layers,
...
def train_PG(
    ...
    discrete = isinstance(lqr_fcn.action_space, gym.spaces.Discrete)

    # Observation and action sizes
    ob_dim = lqr_fcn.observation_space.shape[0]
    ac_dim = lqr_fcn.action_space.n if discrete else lqr_fcn.action_space.shape[0]
    ...
    for itr in range(n_iter):
        ...
        paths, timesteps_this_batch = agent.sample_trajectories(itr, lqr_fcn)
...
# python3 train_pg_f18_nobias.py -ep 1000 --discount 0.98 -n 180 -e 3 -l 2 -s 32 -b 6000 -lr 1e-3 -rtg --nn_baseline --exp_name p_b6000_r1e-3
def main():
    import argparse
